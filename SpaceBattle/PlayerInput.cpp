...
void PlayerInput::update(GameObject& gameobject, IWorld* world)
{
	...
	{
		glm::vec2 forward(cos(helper::deg_to_rad(gameobject.get_rot() - 90.00f)), sin(helper::deg_to_rad(gameobject.get_rot() - 90.00f)));
		gameobject.set_velocity(forward * speed);
	}
	else
	{
		gameobject.set_velocity(gameobject.get_velocity() * 0.99f);
	}
	if (sf::Keyboard::isKeyPressed(sf::Keyboard::Space))
	{
		sf::Time time_now = clock.getElapsedTime();
		if ((time_now.asMilliseconds() - start_time.asMilliseconds()) >= 500)
		{
			...
			/*if (bullet != NULL)
			{
				...
				//bullet->set_velocity(glm::vec2(cos(helper::deg_to_rad(angle - 90)), sin(helper::deg_to_rad(angle - 90))));
